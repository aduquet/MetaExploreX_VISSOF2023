...
def dataExecution(inputTestID, testData, outputData, outputchecker, pathdata, pathChecker):
    ...
    for i in range(0, len(testData)):
        ...
        output_testData = find_magnitude(testInput)
        output_MR1 = find_magnitude(MR1)
        output_MR2 = find_magnitude(MR2)
        output_MR3 = find_magnitude(MR3)
        output_MR5 = find_magnitude(MR5)
        output_MR6 = find_magnitude(MR6)

        if 'N/A' in MR4:
            output_MR4 = 'd/0'
        else:
            output_MR4 = find_magnitude(MR4)
