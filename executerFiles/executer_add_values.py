...
def dataExecution(inputTestID, testData, outputData, outputchecker, pathdata, pathChecker):
    ...
    for i in range(0, len(testData)):
        ...
        output_testData = add_values(testInput)
        output_MR1 = add_values(MR1)
        output_MR2 = add_values(MR2)
        output_MR3 = add_values(MR3)
        output_MR5 = add_values(MR5)
        output_MR6 = add_values(MR6)

        if 'N/A' in MR4:
            output_MR4 = 'd/0'
        else:
            output_MR4 = add_values(MR4)
