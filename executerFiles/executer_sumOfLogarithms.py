...
def dataExecution(inputTestID, testData, outputData, outputchecker, pathdata, pathChecker):
    ...
    for i in range(0, len(testData)):
        ...
        output_testData = sumOfLogarithms(testInput)
        output_MR1 = sumOfLogarithms(MR1)
        output_MR2 = sumOfLogarithms(MR2)
        output_MR3 = sumOfLogarithms(MR3)
        output_MR5 = sumOfLogarithms(MR5)
        output_MR6 = sumOfLogarithms(MR6)

        if 'N/A' in MR4:
            output_MR4 = 'd/0'
        else:
            output_MR4 = sumOfLogarithms(MR4)
